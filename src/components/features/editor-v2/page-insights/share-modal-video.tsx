...
const ShareModalVideo = ({ videoId, ogURL }) => {
    ...
    const handleCopy = (provider) => {
        ...
        container.innerHTML = getEmailEmbedCodeForSimpleVideos(
            ...
            (function(){ try { const { buildPreviewUrl, normalizeMediaUrl } = require('src/utils/media'); const normalized = normalizeMediaUrl(ogURL); const transform = process.env.NEXT_PUBLIC_CLOUDINARY_TRANSFORM_PREFIX || 'https://res.cloudinary.com/dhd6m0fh3/video/upload/c_scale,h_400/e_loop/dl_200,vs_30/'; return buildPreviewUrl(normalized, transform).replace(/\.(mp4|mov|m3u8|webm)$/, '.gif'); } catch (e) { return `https://res.cloudinary.com/dhd6m0fh3/video/upload/c_scale,h_400/e_loop/dl_200,vs_30/${ogURL.split('/').pop().replace('.mp4', '.gif').replace('.mov', '.gif').replace('.m3u8', '.gif').replace('.webm', '.gif')}` } })(),
            provider.value,
        );
...
export default ShareModalVideo;
