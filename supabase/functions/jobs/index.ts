...
serve(async (req) => {
    ...
    try {
        ...
        if (action === "text-to-video" && req.method === "POST") {
            ...
            if (!prompt || !tenant_id)
                return new Response(
                    JSON.stringify({ error: "prompt and tenant_id required" }),
        ...
        if (action === "image-to-video" && req.method === "POST") {
            ...
            if (!prompt || !image_url || !tenant_id)
                return new Response(
                    JSON.stringify({
                        error: "prompt, image_url, tenant_id required",
                    }),
                    { status: 400, headers: corsHeaders },
                );
        ...
        if (action === "poll" && id) {
            ...
            if (result.status === "completed") {
                ...
                return new Response(
                    JSON.stringify({ status: "completed", outputs }),
                    { headers: corsHeaders },
                );
            }
            ...
            return new Response(
                JSON.stringify({ status: result.status, result }),
                { headers: corsHeaders },
            );
        }
