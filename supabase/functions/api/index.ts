...
async function persistOutputs(
    ...
    const outputs: string[] =
        result.outputs || result.video?.url
            ? [result.video.url]
            : result.image?.url
            ? [result.image.url]
            : result.url
...
const MODELS = {
    ...
    "kling-o1-pro-image-to-video": {
        type: "image-to-video",
        resource: "video",
    },
    "kling-o1-master-image-to-video": {
        type: "image-to-video",
        resource: "video",
    },
...
function _pickModel(model: string | undefined, fallback: string) {
    return MODELS[model || fallback] ? model || fallback : fallback;
}
    ...
    try {
        ...
        if (
            ...
            if (!submit.request_id && !submit.data?.request_id) {
                return json(
                    { error: "Muapi submit failed", detail: submit },
                    502,
                );
            }
        ...
        if (
            ...
            const submit = await muapiSubmit(model, {
                prompt,
                image_url,
                ...rest,
            });
        ...
        if (
            ...
            const {
                ...
                prompt,
                image_url,
                model = "kling-o1-standard-image-to-video",
                ...rest
            } = await req.json();
            const submit = await muapiSubmit(model, {
                prompt,
        ...
        if (
            ...
            const submit = await muapiSubmit(endpoint, {
                video_url,
                audio_url,
                ...rest,
            });
        ...
        if (
            ...
            const submit = await muapiSubmit(model || "image-upscale", {
                image_url,
                ...rest,
            });
            const requestId = submit.request_id || submit.data?.request_id;
        ...
        if (
            segments[0] === "images" &&
            segments[1] === "background-remover" &&
            req.method === "POST"
        ) {
            const tenantId = await getTenantFromRequest(req);
            if (!tenantId) return json({ error: "Auth required" }, 401);
            const { user_id, image_url, model, ...rest } = await req.json();
            const submit = await muapiSubmit(model || "background-remover", {
                image_url,
                ...rest,
            });
            const requestId = submit.request_id || submit.data?.request_id;
        ...
        if (
            segments[0] === "videos" &&
            segments[1] === "face-swap" &&
            req.method === "POST"
        ) {
            const tenantId = await getTenantFromRequest(req);
            if (!tenantId) return json({ error: "Auth required" }, 401);
            const { user_id, source_url, target_url, model, ...rest } =
                await req.json();
        ...
        if (segments[0] === "music" && req.method === "POST") {
            ...
            const jobType: JobType =
                model === "suno-remix-music"
                    ? "music-remix"
                    : model === "suno-extend-music"
                    ? "music-extend"
                    : "music-create";
            const submit = await muapiSubmit(model, { prompt, ...rest });
            const requestId = submit.request_id || submit.data?.request_id;
        ...
        if (
            ...
            const {
                ...
                video_url,
                prompt,
                model = "mmaudio-v2-video-to-video",
                ...rest
            } = await req.json();
            const submit = await muapiSubmit(model, {
                video_url,
        ...
        if (
            ...
            if (character_ids?.length) {
                ...
                const desc = (chars || [])
                    .map(
                        (c: any) =>
                            `${c.name}: ${c.static_features || ""} ${
                                c.dynamic_features || ""
                            }`,
                    )
                    .join("; ");
                mergedPrompt = `${prompt}. Characters: ${desc}. Setting: ${
                    ep?.setting || ""
                }.`;
            }
            ...
            if (id && action === "poll") {
                ...
                if (status === "completed") {
                    const resourceType = (job.resource_type ||
                        MODELS[job.model]?.resource ||
                        "video") as ResourceType;
                    const { outputs, resource } = await persistOutputs(
                        job,
                        result,
                        resourceType,
                    );
        ...
        if (segments[0] === "leads" && req.method === "POST") {
            const body = await req.json();
            const tenantId =
                body.tenant_id || (await getTenantFromRequest(req));
            const { data, error } = await supabase
                .from("leads")
                .insert({ ...body, tenant_id: tenantId })
                .select()
        ...
        if (
            ...
            const { email, full_name, tenant_id, redirectTo } =
                await req.json();
            const { data, error } = await supabase.auth.admin.inviteUserByEmail(
                email,
                {
                    ...
                    redirectTo:
                        redirectTo || "https://app.videco.io/auth/login",
                },
            );
            if (error) throw error;
            return json({ user: data.user });
