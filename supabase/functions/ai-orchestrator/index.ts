            ...
            messages: [
                {
                    role: "system",
                    content:
                        "You write short personalized cold outreach video scripts (30-60s spoken). Return strict JSON {greeting, body, cta, fullScript}",
                },
...
async function handleProcessVideo(data: any) {
    ...
    return new Response(
        JSON.stringify({
            success: true,
            muapi_request_id: videoJob.request_id,
        }),
        {
            headers: { "Content-Type": "application/json" },
        },
...
async function handleAiClone(data: any) {
    ...
    if (!audioUrl) {
        throw new Error(
            "No audio provided and no text to synthesize voice from",
        );
    }
    ...
    return new Response(
        JSON.stringify({
            success: true,
            request_id: cloneJob.request_id,
            audio_url: audioUrl,
        }),
        {
            headers: { "Content-Type": "application/json" },
...
async function handlePollMuapi(data: any) {
    ...
    const result = await fetch(
        `${MUAPI_BASE}/predictions/${request_id}/result`,
        {
            headers: { "x-api-key": MUAPI_KEY },
        },
    ).then((r) => r.json());
...
async function handleMuapiWebhook(
    data: any,
    rawBody?: string,
    ...
    if (signatureHeader && MUAPI_WEBHOOK_SECRET) {
        try {
            const hmac = createHmac("sha256", MUAPI_WEBHOOK_SECRET)
                .update(rawBody || JSON.stringify(data), "utf8")
                .digest("hex");
            ...
            if (!verified) {
                console.warn(
                    "[ai-orchestrator] muapi webhook signature invalid",
                );
                return new Response(
                    JSON.stringify({
                        received: false,
            ...
            console.error(
                "[ai-orchestrator] signature verification failed",
                err,
            );
            return new Response(
                JSON.stringify({ received: false, error: "signature_error" }),
                {
                    status: 200,
                    headers: { "Content-Type": "application/json" },
                },
        ...
        console.warn(
            "[ai-orchestrator] muapi webhook signature present but MUAPI_WEBHOOK_SECRET not configured",
        );
    }

    console.log(
        "[ai-orchestrator] handling webhook for request",
        request_id,
        "status",
        status,
    );
    ...
    async function getExistingStatus() {
        if (video_id) {
            const { data: v } = await supabase
                .from("videos")
                .select("media_status, ai_preview")
                .eq("id", video_id)
                .single();
            return v;
        }
    ...
    if (existing) {
        if (
            existing.media_status === "completed" ||
            existing.status === "completed"
        ) {
            console.log(
                "[ai-orchestrator] already completed for request",
                request_id,
                "- ignoring",
            );
            return new Response(
    ...
    async function reliableUpdate(table: string, payload: any, eqClause: any) {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                ...
                console.error(
                    `[ai-orchestrator] DB update attempt ${attempt} failed for ${table} id=${eqClause}`,
                    err,
                );
