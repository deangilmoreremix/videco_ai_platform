...
async function _muapiUpload(file: File): Promise<any> {
...
async function _getTenantFromEvent(event: any): Promise<string | null> {
    const headerTenant =
...
async function _persistOutputs(
    job: any,
    result: any,
    resourceType: string,
...
export const handler: Handler = async (event) => {
    ...
    if (!tenantId) {
        return {
            statusCode: 400,
            headers: corsHeaders,
            body: JSON.stringify({ error: "x-tenant-id required" }),
        };
    }
    ...
    try {
        ...
        if (event.httpMethod === "POST") {
            ...
            if (contentType.includes("multipart/form-data")) {
                ...
                return {
                    ...
                    headers: corsHeaders,
                    body: JSON.stringify({
                        error: "Multipart upload requires pre-processing",
                    }),
                };
            }
            ...
            if (body.url) {
                return {
                    statusCode: 200,
                    headers: corsHeaders,
                    body: JSON.stringify(await muapiUploadFromUrl(body.url)),
                };
            }
        ...
        if (
            url.pathname.includes("/text-to-image") &&
            ...
            return {
                statusCode: 200,
                headers: corsHeaders,
                body: JSON.stringify({ job, muapi: submit }),
            };
        }
        ...
        if (
            url.pathname.includes("/background-remover") &&
            event.httpMethod === "POST"
        ) {
            const body = JSON.parse(event.body || "{}");
            const { user_id, image_url, model, ...rest } = body;
