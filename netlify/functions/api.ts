...
export const handler: Handler = async (event) => {
    ...
    try {
        ...
        if (segments[0] === "webhooks" && segments[1] === "muapi") {
            ...
            const provided =
                event.headers["x-muapi-webhook-secret"] ||
                event.headers["X-Muapi-Webhook-Secret"];
        ...
        if (
            ...
            const tenantId = await getTenantFromEvent(event);
            if (!tenantId) return json({ error: "Auth required" }, 401);
            const { user_id, image_url, model, ...rest } = JSON.parse(
                event.body || "{}",
            );
        ...
        if (segments[0] === "leads" && event.httpMethod === "POST") {
            const body = JSON.parse(event.body || "{}");
            const tenantId =
                body.tenant_id || (await getTenantFromEvent(event));
            const { data, error } = await supabase
                .from("leads")
                .insert({ ...body, tenant_id: tenantId })
                .select()
        ...
        if (
            ...
            const { email, password, full_name, tenant_id } = JSON.parse(
                event.body || "{}",
            );
        ...
        if (
            ...
            const { email, full_name, tenant_id, redirectTo } = JSON.parse(
                event.body || "{}",
            );
