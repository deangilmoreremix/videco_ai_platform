...
const corsHeaders = {
    ...
    "Access-Control-Allow-Headers":
        "Content-Type, Authorization, x-internal-secret, x-muapi-webhook, x-muapi-webhook-secret",
    ...
    try {
        ...
        await fetch(`${process.env.SUPABASE_URL}/rest/v1/usage`, {
            ...
            body: JSON.stringify({
                ...
                action: "generate-tts",
                details: {
                    text_len: (typeof text === "string" ? text : "").length,
                },
                cost_estimate: 0.0,
            }),
        }).catch(() => undefined);
...
async function handleAiClone(data: Json) {
    ...
    if (!audioUrl) {
        return {
            ...
            headers: corsHeaders,
            body: JSON.stringify({
                error: "No audio provided and no text to synthesize voice from",
            }),
        };
    }
...
export const handler: Handler = async (event) => {
    ...
    try {
        switch (payload.action) {
            ...
            case "muapi-webhook":
                return await handleMuapiWebhook(
                    payload,
                    event.body,
