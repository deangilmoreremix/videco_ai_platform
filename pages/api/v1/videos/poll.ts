...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const result = await pollResult(request_id, { maxAttempts: 60, intervalMs: 3000 });

    if (result.status === "completed" && result.outputs?.[0]?.url) {
      const finalUrl = result.outputs[0].url;

      // Reliable update with simple retries
      async function reliableUpdate(table: string, payload: any, id: string) {
        const maxAttempts = 3;
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
          try {
            ...
            console.error(`[videos/poll] DB update attempt ${attempt} failed for ${table} id=${id}`, err);
      ...
      if (video_id) {
        const { data: existing } = await supabase.from("videos").select("media_status").eq("id", video_id).single();
      ...
      if (ai_video_id) {
        const { data: existingAi } = await supabase.from("ai_videos").select("status").eq("id", ai_video_id).single();
        if (existingAi?.status === "completed") {
          console.log("[videos/poll] ai_video already completed", ai_video_id);
          return res.status(200).json({ success: true, ignored: true });
        }
        await reliableUpdate("ai_videos", { status: "completed", url: finalUrl }, ai_video_id);
      }

      return res.status(200).json({ success: true, status: "completed", final_url: finalUrl });
    }
